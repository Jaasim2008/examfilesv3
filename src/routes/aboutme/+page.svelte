...
<div class="p-10 text-lg grid grid-cols-1 gap-6 md:w-4/6 mx-auto">
	{#each [
		...
		<div class="border-b border-gray-300 group">
			<button on:click={() => toggle(index)} class="w-full text-left px-4 py-3 flex items-center justify-between hover:bg-gray-100 focus:outline-none" >
				<span class="font-medium text-accent group-hover:text-primary">{title}</span>
				<svg class="w-5 h-5 text-accent transform transition-transform duration-300" class:rotate-180={openIndex === index} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" >
			...
			<div class="accordion-content px-4" style="max-height: {openIndex === index ? 'full' : '0px'}; padding-top: {openIndex === index ? '1rem' : '0'}; padding-bottom: {openIndex === index ? '1rem' : '0'}">
				...
				{:else if index === 4}
					...
					<table class="md:w-2/6 border border-primary table-auto text-sm mt-4">
						...
						<tbody>
							...
							{:else}
								{#each data as code}
									<tr>
										<td class="border border-primary px-4 py-2">{code.abbr}</td>
										<td class="border border-primary px-4 py-2">{code.meaning}</td>
									</tr>
								{/each}
							{/if}
						</tbody>
	...
	<a class="text-primary underline mt-6" href="/">&lt; return</a>
</div>
